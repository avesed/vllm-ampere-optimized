...
INT8_BRANCH = r'''  uint32_t a_frag[KTraits::NUM_MMA_Q][4], b_frag[4];
  ...
  if constexpr (sizeof(typename KTraits::DTypeQ) == 1) {
    ...
    auto ldq = [&](uint32_t row, uint32_t dcol) -> uint32_t {
      ...
      return off * 2654435761u + 1013904223u;  // NOLOAD probe: arith hash, no smem read
    };
    auto ldk = [&](uint32_t row, uint32_t dcol) -> uint32_t {
      uint32_t jb = dcol >> 4, e = dcol & 15;
      uint32_t off = (row * US_K + (jb ^ (row & 7u))) * 16u + e;
      return off * 40503u + 12345u;  // NOLOAD probe: arith hash, no smem read
    };
    ...
    float qs_lo_a[KTraits::NUM_MMA_Q], qs_hi_a[KTraits::NUM_MMA_Q];
#pragma unroll
...
#pragma unroll
    for (uint32_t mk = 0; mk < KTraits::NUM_MMA_KV; ++mk) {
      ...
      ks0a_a[mk] = kscale(kv0); ks0b_a[mk] = kscale(kv0 + 1);
      ks1a_a[mk] = kscale(kv1); ks1b_a[mk] = kscale(kv1 + 1);
    }
#pragma unroll
...
open(P, "w").write(s)
print("I4_NOLOAD_PROBE_APPLIED: per-token dequant int8 compute_qk + 3 call sites + scale accessors")
