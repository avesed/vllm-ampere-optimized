...
| `0007-vllm-serve-autotune.patch` | **Pure-Python.** Adds `vllm serve <args> --autotune[...]` — two thin lazy-import hooks in `entrypoints/cli/serve.py` (`cmd()` intercept + `subparser_init` arg-group via `ampere_autotune.serve_autotune.add_autotune_args`) that hand off to the `ampere_autotune` HALF-A flag-autotuner, **reusing the user's OWN serve args** (model/TP/port/temperature/…) as the per-config restart template (reconstructed from `argv` minus the `--autotune*` flags; the swept child is relaunched by **pidfile** so it never kills the orchestrator, whose cmdline also contains `vllm serve`). Docker-friendly: identical `vllm serve` command shape + one flag, **no entrypoint/CMD change**. Imports are lazy + `try/except`, so a normal `vllm serve` is unaffected when `ampere_autotune` is absent. **Requires the `autotune/` package pip-installed in the image for the feature to activate** (HALF-A is no-privilege; HALF-B silicon stays host-side). Applied by `git apply -p1 --directory=vllm`. |
