...
- **DiffusionGemma (block-diffusion dLLM, the row above):** single-stream gen tok/s, canvas 256. int8-act also
  beats **NVFP4 (176)** — no native FP4 kernel below `sm_89`, so it dequant-emulates to bf16 (memory-only, ≈ W4A16
  speed) — at matched accuracy: W4A8 GSM8K 95.7% / MMLU-Pro 76.4%; NVFP4 96.0% / 77.8% (N=1000/500). Two serve
