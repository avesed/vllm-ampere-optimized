...
torch::stable::Tensor moe_wna16_marlin_gemm(
  ...
  if (global_scale_or_none.has_value()) {
    ...
    // Ampere fork (patch 0005): global_scale carries the per-expert reconstruction
    // factor for nvfp4 AND for int8-act MoE (per-expert (1/4096)*max); both index
    // global_scale_ptr[expert_id] in the kernel.
  ...
  // Ampere fork (patch 0005): pass nullptr when no per-expert global_scale was given
  // so the kernel keeps global_scale_f32 = 1.0 (its runtime null-guard handles it).
  void* global_scale_kernel_ptr =
      global_scale.numel() > 0 ? global_scale.mutable_data_ptr() : nullptr;
  MARLIN_NAMESPACE_NAME::marlin_mm(
      ...
      global_scale_kernel_ptr, b_zeros.mutable_data_ptr(),
