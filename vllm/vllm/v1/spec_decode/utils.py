...
def copy_and_expand_dspark_inputs_kernel(
    ...
    Identical to copy_and_expand_dflash_inputs_kernel for positions,
    slot-mapping, and input_ids (anchor bonus token at query slot 0).
    DSpark differs only in the sampled-slot set: it uses gamma query
    slots (not 1 + gamma) and samples ALL gamma slots including the
    anchor at slot 0 (next-token-shift readout), whereas DFlash samples
    only the gamma mask slots and skips the bonus slot.
    """
    req_idx = tl.program_id(axis=0)
    block_idx = tl.program_id(axis=1)
    ...
    # --- Token indices to sample (DSpark: sample ALL gamma slots incl. anchor) ---
    is_sample = is_query
