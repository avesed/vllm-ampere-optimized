...
class SpecDecodeBaseProposer:
    ...
    def propose(
        ...
        if self.num_speculative_tokens == 1 or self.parallel_drafting:
            # DSpark: when the draft model carries a Markov head (markov_rank>0),
            # sample the parallel block SEQUENTIALLY with the per-position markov
            # bias instead of the one-shot independent sample. markov_rank=0
            # (plain DFlash) keeps the original path.
            if getattr(self.model, "markov_rank", 0) > 0:
                base_logits = self.model.compute_logits(sample_hidden_states).reshape(
                    batch_size, self.num_speculative_tokens, -1
                )
                return self.model.markov_sample_block(base_logits, next_token_ids)
    ...
    def initialize_attn_backend(
        ...
        # _draft_attn_layer_names is a set; iterate in sorted order so that
        # attention_groups (and anything derived from its first element) is
