...
struct KernelTraits {
  ...
  static constexpr bool IsInvalid() {
    return ((NUM_MMA_D_VO < 4) || (NUM_MMA_D_VO == 4 && NUM_MMA_KV % 2 == 1) ||
            ...
            (NUM_MMA_Q * (4 * NUM_MMA_D_VO + 2 * sizeof(DTypeQKAccum) * NUM_MMA_KV) >= 256) ||
...
template <typename KTraits>
inline constexpr bool FA_PV16 = (FA_USE_FP16_PV != 0) && (std::is_same_v<typename _fa_ptype<KTraits>::type, half> || (std::is_void_v<typename _fa_ptype<KTraits>::type> && std::is_same_v<typename _fa_qtype<KTraits>::type, half>));
template <typename KTraits>
...
#pragma unroll
        for (uint32_t mma_do = 0; mma_do < KTraits::NUM_MMA_D_QK / (8 / sizeof(DTypeQ)); ++mma_do) {
        ...
        q_smem_offset_w =
            ...
            sizeof(DTypeQ) * KTraits::NUM_MMA_D_QK;
      }
    }
  }
}
...
__device__ __forceinline__ void compute_qk(
  ...
  if constexpr (sizeof(typename KTraits::DTypeQ) == 1) {
    ...
    // ====== TUNED scale handling (register-pressure driven) ======
    // The kernel is REGISTER-BOUND at 255 regs WITH stack spill (cuobjdump shows STACK:48-320
    // on the f16 path AND the int8 path). A scaleconst probe (real smem loads + CONSTANT scales,
    // i.e. zero scale registers) hit 1.08-1.10x@D256 / 1.20x@D128 vs the un-tuned baseline
    // 0.96x@D256 — proving the SCALE-REGISTER PRESSURE, not the smem load, is the bottleneck
    // (noload probe = same 1.09-1.12x ceiling). So: keep ALL scale state OFF the load+mma loop.
    //   * k-scales: never pre-expanded (16 regs); read 4 __ldg per mma_kv in the dequant epilogue.
    //   * q-scales: D-dependent. The divmod-vs-prearray trade flips with head-dim k-tile count:
    //       - D256+ (HD/32>=8, the most register-critical): compute inline in the epilogue
    //         (DEFER_QSCALE) so NOTHING q-related lives during the loop -> +D256 (1.03/1.07).
    //       - D128 (HD/32==4, less pressure): pre-array up-front (4 regs) so the GQA divmod is
    //         off the epilogue -> +D128 (1.10/1.08). Inlining the divmod hurt D128 (0.98/0.93).
    constexpr bool DEFER_QSCALE = (HD >= 256);
    float qs_lo_a[DEFER_QSCALE ? 1 : KTraits::NUM_MMA_Q];
    float qs_hi_a[DEFER_QSCALE ? 1 : KTraits::NUM_MMA_Q];
    if constexpr (!DEFER_QSCALE) {
#pragma unroll
      for (uint32_t mq = 0; mq < KTraits::NUM_MMA_Q; ++mq) {
        qs_lo_a[mq] = qscale(qo_packed_idx_base + mq * 16 + g);
        qs_hi_a[mq] = qscale(qo_packed_idx_base + mq * 16 + g + 8);
      }
    }
#pragma unroll
    for (uint32_t mma_q = 0; mma_q < KTraits::NUM_MMA_Q; ++mma_q) {
...
#pragma unroll
        ...
        float qs_lo, qs_hi;
        if constexpr (DEFER_QSCALE) {
          qs_lo = qscale(qo_packed_idx_base + mma_q * 16 + g);
          qs_hi = qscale(qo_packed_idx_base + mma_q * 16 + g + 8);
        } else {
          qs_lo = qs_lo_a[mma_q];
          qs_hi = qs_hi_a[mma_q];
        }
        const uint32_t kv0 = kv_idx_base + mma_kv * 16 + 2 * t;
        const uint32_t kv1 = kv_idx_base + mma_kv * 16 + 8 + 2 * t;
        const float ks0a = kscale(kv0), ks0b = kscale(kv0 + 1);
        const float ks1a = kscale(kv1), ks1b = kscale(kv1 + 1);
...
__device__ __forceinline__ void compute_sfm_v(
  ...
  typename KTraits::DTypeProb s_frag_f16[KTraits::NUM_MMA_Q][KTraits::NUM_MMA_KV][8];
  if constexpr (std::is_same_v<typename KTraits::DTypeQKAccum, float>) {
#pragma unroll
    for (uint32_t mma_q = 0; mma_q < KTraits::NUM_MMA_Q; ++mma_q) {
#pragma unroll
      for (uint32_t mma_kv = 0; mma_kv < KTraits::NUM_MMA_KV; ++mma_kv) {
        vec_cast<typename KTraits::DTypeProb, float>::cast<8>(s_frag_f16[mma_q][mma_kv],
                                                           s_frag[mma_q][mma_kv]);
      }
    }
  }
...
__device__ __forceinline__ void SinglePrefillWithKVCacheDevice(
  ...
  if constexpr (std::is_same_v<DTypeQ, nv_bfloat16>) {
    ...
    static_assert(sizeof(DTypeQ) == 2 || sizeof(DTypeQ) == 1);
    ...
    init_states<KTraits>(variant, o_acc, m, d);
...
#pragma unroll 1
    for (uint32_t iter = 0; iter < num_iterations; ++iter) {
      ...
      update_mdo_states<KTraits>(variant, s_frag, o_acc, m, d);
...
#pragma unroll
          for (uint32_t _r = 0; _r < 8; ++_r) o_frag[_mq][_md][_r] = ((float*)o_acc[_mq][_md])[_r];
        }
      }
