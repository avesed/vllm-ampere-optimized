...
# famp Ampere W4A8/W4A16 SUBSET: only the int4-weight configs famp serves — W4A16 (fp16/bf16
# activation) + W4A8 (int8/kS8 activation), for AWQ (kU4) + GPTQ (kU4B8) int4 weights. Dropped
# vs upstream: kU8B128 (W8A16 int8-weight, not a famp quant), and all FP8/NVFP4/MXFP + fp8-act
# configs (sm80/sm86 have no fp8 tensor cores — generate_kernels skips them anyway). This cuts the
# instantiation count ~3x for a tractable standalone build. group_blocks keeps -1 (channelwise) +
# 2/4/8 (g32/g64/g128) — famp recipe is g32 (group_blocks=2).
QUANT_CONFIGS = [
    # AWQ-INT4  (W4A16: fp16/bf16 act)
    {
        "b_type": "kU4",
    ...
    # AWQ-INT4 with INT8 activation  (W4A8 — the famp flagship)
    {
        "a_type": ["kS8"],
        "b_type": "kU4",
        "thread_configs": THREAD_CONFIGS,
    ...
    # GPTQ-INT4 with INT8 activation  (W4A8 — the famp flagship)
    {
        "a_type": ["kS8"],
        "b_type": "kU4B8",
        "thread_configs": THREAD_CONFIGS,
...
# MARLIN_MINIMAL=1 -> a tiny set (one GPTQ-INT4 W4A16 config, g32 only, m=1/2) to isolate
# linker size/relocation issues from the full subset. NOT for production.
if os.environ.get("MARLIN_MINIMAL"):
    QUANT_CONFIGS = [
        {
            "b_type": "kU4B8",
